...
def user_selection_save(request):
    ...
    # ------- STEP 1 -- Create a ProductForm instance
    ...
    if request.method == 'POST':
        ...
        if selected_products:
            ...
            save_selection.productsList = selected_products
            save_selection.save()
            return redirect('/choose_solver/')

    # Render the template with the form (for both GET and POST if validation fails)
...
def show_path(request):
    ...
    # ------- STEP 3 -- Generate graph containing cells as keys and the neighboring cells we can visit as values
    ...
    # ------- STEP 4 -- Recover last user's selection as a list and assign coordinates to each product
    ...
    # ------- STEP 5 -- Generate the graph model and solve the problem
